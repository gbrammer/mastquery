...
FILTER_EXAMPLES = [
    {"paramName": "productLevel", "values": ["2a", "2b"]},
    # {'paramName': 'filetype',
    #  'values': ['countrate','calibrated']},
    {"paramName": "program", "values": ["1076"]},
    {"paramName": "expstart", "values": [{"min": 59672.0, "max": 59690.0}]},
    {"paramName": "filename", "values": [], "freeText": "%rate.fits"},
]
...
CALIB_FILTERS = [
    ...
    # {'paramName': 'filetype',
    #    'values': ['countrate','calibrated']},
    {"paramName": "filename", "values": [], "freeText": "%_[cr]a%[le].fits"},
]
...
def load_column_lists():
    """
    Read lists of column names
    """
    column_file = os.path.join(os.path.dirname(__file__), "data", "mast_columns.yaml")
...
def make_program_filter(progs):
    ...
    """
    return make_query_filter(column="program", values=[f"{int(p)}" for p in progs])


def query_all_jwst(
    instruments=["NRC", "NIS", "NRS", "MIR"],
    columns=None,
    rd=None,
    sort="expstart",
    fix=True,
    **kwargs,
    ...
    for t in res:
        ...
        for c in t.colnames:
            if t[c].dtype in [object]:
                ...
                t[c] = np.ma.masked_array(
                    t[c], mask=fill, dtype=t[c][~fill][0].__class__
                )
    ...
    if rd is not None:
        ...
        if len(rd) == 2:
            ...
            idx, dr = tab.match_to_catalog_sky(jw, other_radec=("prop_ra", "prop_dec"))
...
def query_jwst(
    instrument="NIS",
    columns="*",
    filters=CALIB_FILTERS + FULL_SUBARRAY + FINE_GUIDE,
    extra={"format": "json", "pagesize": 100000},
    recent_days=None,
    ...
    **kwargs,
    ...
    if recent_days is not None:
        ...
        filters.append(
            {
                "paramName": "expstart",
                "values": [{"min": float(now - recent_days), "max": float(now + 1)}],
            }
        )
    ...
    request["params"] = {"columns": columns, "filters": filters}
    for k in extra:
        request[k] = extra[k]
    ...
    if ("program" in tab.colnames) & ("pi_name" in tab.colnames):
        tab["prog_pi"] = [
            f"{ca:>4}-{pr} {pi}"
            for ca, pr, pi in zip(tab["category"], tab["program"], tab["pi_name"])
        ]

    if ("filter" in tab.colnames) & ("pupil" in tab.colnames):
        tab["filter-pupil"] = [
            f"{f}-{p}".replace("---", "") for f, p in zip(tab["filter"], tab["pupil"])
        ]

        tab["inst-mode"] = [
            f"{ii}-{f}-{p}".replace("---", "").replace("-CLEAR", "")
            for ii, f, p in zip(tab["instrume"], tab["filter"], tab["pupil"])
        ]
    elif "filter" in tab.colnames:
        tab["filter-pupil"] = [f"{f}".replace("---", "") for f in tab["filter"]]

        tab["inst-mode"] = [
            f"{ii}-{f}".replace("---", "").replace("-CLEAR", "")
            for ii, f in zip(tab["instrume"], tab["filter"])
        ]
...
def query_guidestar_log(mjd=None, program=None, exp_type=["FGS_FINEGUIDE"]):
    ...
    gs = query_jwst(instrument="GS", columns="*", filters=filters, extensions=["cal"])
